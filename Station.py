...
for station in liste_station:
    for station_proche in liste_station:
        if station['name'] != station_proche["name"]:
            distance = math.sqrt((station["longitude"]-station_proche["longitude"]) * (station["longitude"]-station_proche["longitude"]) + (station["latitude"] - station_proche["latitude"]) * (station["latitude"] - station_proche["latitude"]))
